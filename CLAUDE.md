...
- Charte éditoriale **versionnée par workspace** (`charter_versions`, append-only, version la plus haute = active). Éditable dans `/settings` (`charter-versions.ts` + `charter-actions.ts`) ; rollback = ré-append. La génération utilise la charte active du workspace ; `TDS_CHARTER` (constante `charter.ts`) sert de seed v1 + fallback. **Overlay par réseau** (US-2.5) : `workspaces.network_charters` (jsonb, éditable dans `/settings`) fusionné à la charte de base via `network-charter.ts` selon le réseau cible de la com (`communications.network`, choisi à la création). Génération + régénération appliquent le même overlay.
